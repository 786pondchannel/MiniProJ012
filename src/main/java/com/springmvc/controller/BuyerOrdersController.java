...
public class BuyerOrdersController {
    ...
    /* ====== ใช้เส้นทางอัปโหลดเดียวกับ WebConfig (/uploads/**) ====== */
    ...
    private static final Path RECEIPTS_DIR  = UPLOAD_ROOT.resolve("receipts");
    private static final Path FARMER_QR_ROOT= UPLOAD_ROOT.resolve("slip");
    ...
    public String uploadReceipt(@PathVariable String orderId,
        ...
        try (Session s = HibernateConnection.getSessionFactory().openSession()) {
            ...
            // เก็บค่าแบบสัมพัทธ์ (relative) เพื่อแมปเป็น /uploads/<path> ตอนแสดงผล
    ...
    public ResponseEntity<String> getReceiptJson(@PathVariable String orderId, HttpSession session){
        ...
        try (Session s = HibernateConnection.getSessionFactory().openSession()) {
            ...
            // ถ้าไฟล์มีจริง ให้ชี้เป็น static /uploads/...
            Path file = resolveReceiptPath(imgRaw);
            if (file != null && Files.isRegularFile(file)) {
                String imgClean = imgRaw.replace("\\","/").replaceFirst("^/+","");
                imgClean = imgClean.replaceFirst("^uploads/",""); // กันซ้ำ
                String staticUrl = "/uploads/" + imgClean;
                return ResponseEntity.ok("{\"url\":\"" + esc(staticUrl) + "\"}");
            }

            // Fallback: โหลดผ่าน endpoint สตรีม (กันกรณีไฟล์เก่าอยู่ path แปลก)
    ...
    public ResponseEntity<String> getFarmerQrJson(@PathVariable String orderId, HttpSession session){
        ...
        try (Session s = HibernateConnection.getSessionFactory().openSession()) {
            ...
            try (var st = Files.list(dir)) {
                ...
                if (any.isPresent()) {
                    // ชี้เป็น static /uploads/slip/<farmerId>/<ไฟล์>
                    String fileName = any.get().getFileName().toString();
                    String url = "/uploads/slip/" + esc(farmerId) + "/" + esc(fileName);
                    return ResponseEntity.ok("{\"url\":\"" + url + "\"}");
                }
            } catch (Exception ignore) { /* ถ้าไม่มีโฟลเดอร์/ไฟล์ให้ fallback */ }

            // Fallback endpoint แบบสตรีม
        ...
        try (Session s = HibernateConnection.getSessionFactory().openSession()) {
            ...
            switch (a){
                ...
                case "REQUEST_PAYMENT_QR", "REQUEST_PAYMENT" -> {
                    // ชี้ไฟล์ QR แบบ static ถ้ามี ไม่งั้นให้ endpoint stream เป็น fallback
                    Path dir = FARMER_QR_ROOT.resolve(farmerId);
                    String imageUrl;
                    try (var st = Files.list(dir)) {
                        Optional<Path> any = st.filter(Files::isRegularFile).findFirst();
                        if (any.isPresent()) {
                            imageUrl = "/uploads/slip/" + esc(farmerId) + "/" + esc(any.get().getFileName().toString());
                        } else {
                            imageUrl = "/orders/" + esc(orderId) + "/payment-qr/image?ts=" + now;
                        }
                    } catch (Exception ex) {
                        imageUrl = "/orders/" + esc(orderId) + "/payment-qr/image?ts=" + now;
                    }
