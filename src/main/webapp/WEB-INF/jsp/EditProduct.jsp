  ...
  <div class="max-w-6xl mx-auto mt-6 px-4 space-y-3">
    ...
    <c:if test="${not empty msg}">
      <div class="alert alert-success" id="flashOk">
        ...
        <div class="flex-1">
          <strong>สำเร็จ:</strong> <c:out value="${msg}"/>
        </div>
    ...
    <c:if test="${not empty error}">
      <div class="alert alert-error" id="flashErr">
        ...
        <div class="flex-1">
          ...
          <c:if test="${not empty errList}"><button type="button" class="ml-3 underline hover:opacity-80" onclick="toggleErrorDetails()">ดูรายละเอียด</button></c:if>
        </div>
  ...
  <main class="container mx-auto px-4 py-8 grid xl:grid-cols-3 gap-8">
    ...
    <div class="xl:col-span-2 space-y-8">
      ...
      <div class="card p-6">
        <div class="flex items-start justify-between">
          <div>
            ...
            <div class="hint mt-1">เลือกรูปหลายไฟล์ได้ สูงสุด 10 ไฟล์ (.JPG .PNG .WEBP ไม่เกิน ~5MB/ไฟล์)</div>
          </div>
          <span class="text-sm border rounded-full px-3 py-1 bg-white">เลือกแล้ว: <span id="countNow">0</span> / 10</span>
        </div>
                  ...
                  <c:forEach var="im" items="${images}">
                    <div class="thumb h-24">
                      ...
                      <label class="thumb-btn cursor-pointer">
                        <input type="checkbox" name="deleteImageIds" value="${im.imageId}" class="mr-1" form="productForm"> ลบ
                      </label>
                    </div>
                  </c:forEach>
                </div>
      ...
      <div class="card p-6">
        ...
        <!-- ====== เตรียม/Normalize สถานะพรีออเดอร์ ====== -->
        ...
        <form id="productForm" action="${formAction}" method="post" enctype="multipart/form-data" class="space-y-6">
          ...
          <div class="alert alert-info items-center">
            ...
            <div class="flex-1">
              ระบบจะคำนวณ “เปิดขาย/ปิดขาย” ให้อัตโนมัติจาก <b>สถานะพรีออเดอร์</b> + <b>สต๊อก</b>
            </div>
            ...
            <div class="md:col-span-2">
              ...
              <div class="igroup">
                ...
                <textarea name="description" rows="5"
                          ...
                          maxlength="1000"
                          placeholder="คำอธิบาย จุดเด่น วิธีการปลูก/เลี้ยง รอบการเก็บเกี่ยว ฯลฯ"><c:out value='${p.description}'/></textarea>
              </div>
          ...
          <div class="flex items-center justify-between pt-2">
            ...
            <button type="submit" class="btn btn-save">
              ...
              <c:choose><c:when test="${empty p.productId}">บันทึก</c:when><c:otherwise>แก้ไข</c:otherwise></c:choose>
            </button>
          </div>
        </form>
      </div>
    ...
    <!-- พรีวิว (ขวา 1 คอลัมน์) -->
    <div class="card p-6">
      <div class="flex items-center justify-between mb-2">
        <div class="text-gray-700 font-semibold">พรีวิวรูปใหม่</div>
        <div class="text-xs text-gray-500">คลิกรูปเพื่อลบ (รูปใหม่) | ลาก-วางได้</div>
      </div>
...
[
  ...
  <script>
    ...
    function paintSaleMode(){
      ...
      let text='โหมดการขาย: -', cls='inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border bg-gray-50 text-gray-700 border-gray-200';
      if (st === 'พร้อมสั่งซื้อแล้ว' && stock > 0){
